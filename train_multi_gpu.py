...
def train():
    with tf.device("/cpu:0"):
        images = tf.placeholder(tf.float32, [None, config.input_shape, config.input_shape, 3])
        lr_images = tf.placeholder(tf.float32, [None, config.input_shape // 4, config.input_shape // 4, 3])
        grid_shapes = [config.input_shape // 32, config.input_shape // 16, config.input_shape // 8]

        bbox_true_13 = tf.placeholder(tf.float32, [None, grid_shapes[0], grid_shapes[0], 3, 5 + config.num_classes])
        bbox_true_26 = tf.placeholder(tf.float32, [None, grid_shapes[1], grid_shapes[1], 3, 5 + config.num_classes])
        bbox_true_52 = tf.placeholder(tf.float32, [None, grid_shapes[2], grid_shapes[2], 3, 5 + config.num_classes])
                ...
                with tf.device(d):
                    with tf.name_scope("tower_%d" % i):
                        ...
                        with tf.variable_scope("model_gd"):
                            images_tower = images[i * config.train_batch_size : (i + 1) * config.train_batch_size]
                            lr_images_tower = lr_images[i * config.train_batch_size : (i + 1) * config.train_batch_size]
                            # bbox_true_tower = [tmp[i * config.train_batch_size : (i + 1) * config.train_batch_size, :] for tmp in bbox_true]
                            bbox_true_13_tower = bbox_true_13[i * config.train_batch_size : (i + 1) * config.train_batch_size]
                            bbox_true_26_tower = bbox_true_26[i * config.train_batch_size : (i + 1) * config.train_batch_size]
                            bbox_true_52_tower = bbox_true_52[i * config.train_batch_size : (i + 1) * config.train_batch_size]
                            # bbox_true = [bbox_true_13, bbox_true_26, bbox_true_52]
                            bbox_true_tower = [bbox_true_13_tower, bbox_true_26_tower, bbox_true_52_tower]


                            model = yolo(config.norm_epsilon, config.norm_decay, config.anchors_path, config.classes_path, config.pre_train)
                            net_g1 = model.GAN_g(lr_images_tower, is_train=True, mask=False)
                            tf.get_variable_scope().reuse_variables()
                            # net_g = model.GAN_g(lr_images, is_train=True, reuse=True, mask=True)

                            d_real = model.yolo_inference(images_tower, config.num_anchors / 3, config.num_classes, training=True)
                            tf.get_variable_scope().reuse_variables()
                            ...
                            # ---------------------------d_loss---------------------------------
                            d_loss1 = model.yolo_loss(d_real, bbox_true_tower, model.anchors, config.num_classes, 1,
                                                      config.ignore_thresh)
                            d_loss2 = model.yolo_loss(d_fake, bbox_true_tower, model.anchors, config.num_classes, 0,
                                                      config.ignore_thresh)
                            ...
                            mse_loss1 = tl.cost.mean_squared_error(net_g1.outputs, images_tower, is_mean=True)
                            ...
                            clc_loss = model.yolo_loss(d_fake, bbox_true_tower, model.anchors, config.num_classes, 1,
                                                       config.ignore_thresh)
        ...
        with tf.Session(config=tf.ConfigProto(log_device_placement=False, allow_soft_placement=True)) as sess:
            ...
            dloss_value = 0
            gloss_value = 0
            train_data = train_reader.build_dataset(config.train_batch_size * 2)
            iterator = train_data.make_one_shot_iterator()
            images_, bbox_, bbox_true_13_, bbox_true_26_, bbox_true_52_ = iterator.get_next()
            lr_images_ = tf.image.resize_images(images_,
                                                size=[config.input_shape // 4, config.input_shape // 4],
                                                method=0, align_corners=False)
            images_in, lr_images_in, bbox_true_13_in, bbox_true_26_in, bbox_true_52_in = sess.run([images_, lr_images_, bbox_true_13_, bbox_true_26_, bbox_true_52_])

            for epoch in range(config.Epoch):
                for step in range(int(config.train_num / (config.train_batch_size * 2))):
                    start_time = time.time()
                    train_dloss, train_gloss, global_step_value, _, _= sess.run([d_loss, g_loss, global_step, train_opd1, train_opg], feed_dict={is_training: True, images:images_in, lr_images:lr_images_in, bbox_true_13:bbox_true_13_in, bbox_true_26:bbox_true_26_in, bbox_true_52:bbox_true_52_in})
                    # train_gloss, global_step_value, _ = sess.run([g_loss, global_step, train_opg], feed_dict={is_training: True, images:images_in, lr_images:lr_images_in, bbox_true_13:bbox_true_13_in, bbox_true_26:bbox_true_26_in, bbox_true_52:bbox_true_52_in})
